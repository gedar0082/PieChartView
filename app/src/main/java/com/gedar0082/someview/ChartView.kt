...
class ChartView @JvmOverloads constructor(
    ...
    //объекты, использующиеся в draw инициализирую здесь чтобы не тратилось время на выделение памяти в onDraw
    //
    //путь для рисования обводки
    private val circlePath: Path = Path()
    //массив путей для рисования частей графика. Переиспользовать один путь не получилось, поэтому создал сразу их по числу частей графика.
    ...
    //массив углов в градусах, которые я получу после нормализации первичных данных и перевода в углы
    ...
    init {
        ...
        var outlineWidthFromAttr : Int = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, outlineWidth, displayMetrics).toInt()
        var outlineColorFromAttr: Int = outlineColor

        //проверяем наличие значений в аттрибутах
        if (attrs != null){
            val typedArray : TypedArray = context.obtainStyledAttributes(attrs, R.styleable.ChartView)
            outlineWidthFromAttr = typedArray.getDimensionPixelSize(R.styleable.ChartView_outlineWidth, outlineWidthFromAttr)
            outlineColorFromAttr = typedArray.getColor(R.styleable.ChartView_outlineColor, outlineColorFromAttr)
            typedArray.recycle()
        }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        //он заполняется здесь, так как если заполнять в конструкторе, то из кода потом значения не поменять
        ...
        circlePath.reset()
        //задаю радиус, который является половиной от размера вьюшки. При построении круга дополнительно вычитаю толщину обводки.
        ...
        //рисую круг
        circlePath.addCircle(measuredHeight/2f, measuredWidth/2f, radius-outlineWidth, Path.Direction.CCW)
